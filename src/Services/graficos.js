...
function FrecuenciaAcumulado(data) {
  ...
  dataFrecAcum.map((dataFrecAcum, index) => {
    ...
    return (
      ...
      (dataFrecAcum["Frecuencia_Ejecutada"] = data.filter((data)=>{return(data["Inicio program."]===dataFrecAcum["Fecha"] && data["Status usuario"] === status)}).length),
      ...
      acumulador_previsto = acumulador_previsto + dataFrecAcum["Frecuencia_Prevista"],
      (dataFrecAcum["Acumulado_Ejecutado"] = acumulador_ejecutado),
      acumulador_ejecutado = acumulador_ejecutado + dataFrecAcum["Frecuencia_Ejecutada"],
